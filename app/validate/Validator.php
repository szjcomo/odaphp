...
class Validator 
{
	...
	private function check($name, $rules, $message) {
        ...
        foreach ($rules as $i => $rule) {
        	...
        	$result = $this->checkResult($rule, $validate_data,$rule_name);
        	if(!$result) {
     ...
     * @return [type]                [boolean]
     */
	private function checkResult($rule, $validate_data,$rule_name) {
        switch ($rule) {
        	case 'require':
        	...
        	default:
        	        if(strpos($rule, ':')) {
        		        ...
        		        return call_user_func_array([$this, $rule_arr[0]], [$func_name, $validate_data,$rule_name]); 
        		    }else{
        		    	return call_user_func_array([$this, $rule], [$rule, $validate_data,$rule_name]); 
        		    }
        		break;
        }
	}
     ...
     * @return     [type]           [description]
     */
    public static function unique($rule,$data,$rule_name)
    {
        if(empty($rule)) return false;
        $id = AppMysql::name($rule)->where([$rule_name=>$data])->value('id');
	 ...
	 * @return [type]       [boolean]
	 */
	public static function length($rule, $value) 
    {
        if (is_array($value)) {
            $length = count($value);
